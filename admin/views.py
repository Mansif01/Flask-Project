...
def generate_winning_draw():
    ...
    new_winning_draw = Draw(user_id=1, numbers=winning_numbers_string, master_draw=True, lottery_round=lottery_round,
                            draw_number_key=current_user.draw_number_key)
...
def view_winning_draw():
    ...
    if current_winning_draw:
        if current_winning_draw:
            # Make the entire winning draw transient before decrypting it
            make_transient(current_winning_draw)

            # Split draw numbers into a list
            draw_numbers_list = current_winning_draw.numbers.split()

            # Decrypt each draw number
            for draw_number in draw_numbers_list:
                current_winning_draw.view_draw_number(current_user.draw_number_key)
...
# This role gives the details of user's information and login and registration time and date when View User Activity
# button is clicked
...
def view_user_activity():
    ...
    return render_template('admin/admin.html', name="PLACEHOLDER FOR FIRSTNAME", user_activity=user_activity)
