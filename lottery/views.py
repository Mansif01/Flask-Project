...
def view_draws():
    ...
    if len(playable_draws) != 0:
        # Making each unplayed draw transient before decrypting it
        for draw_number in playable_draws:
            make_transient(draw_number)
            creator = User.query.filter_by(id=draw_number.user_id).first()
            draw_number.view_draw_number(creator.draw_number_key)
...
def check_draws():
    ...
    if len(played_draws) != 0:
        # The for loop make each played draw number transient before decrypting
        for draw_number in played_draws:
            make_transient(draw_number)
            creator = User.query.filter_by(id=draw_number.user_id).first()
            draw_number.view_draw_number(creator.draw_number_key)
